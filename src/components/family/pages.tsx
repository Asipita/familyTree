...
import { canWrite, canEditStory, canEditProfile, canInvite, connectedPeople, initials, lifespan, relationTo, reviewStory, type Person, type Story } from "@/lib/family";
...
export function ReviewsPage({ initialStory }: { initialStory?: string }) {
  ...
  async function decide(decision: "Approved" | "Changes requested") { if (!story) return; if (await update(s => reviewStory(s, story.id, decision, note))) { setMessage(decision === "Approved" ? "Review saved. The story is now published." : "Your feedback is saved. The story is back with its writer."); setNote(""); } }
