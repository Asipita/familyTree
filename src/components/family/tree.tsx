...
import { canWrite, canInvite, canEditProfile, connectedPeople, initials, lifespan, relationTo, relativeLink, type Person, type RelativeKind } from "@/lib/family";
...
function AddRelative({ anchor, onClose, onAdded }: { anchor: string; onClose: () => void; onAdded: (id: string) => void }) {
  ...
  async function submit(e: React.FormEvent<HTMLFormElement>) { e.preventDefault(); const data = new FormData(e.currentTarget); const name = String(data.get("name")).trim(); const born = String(data.get("born")); const died = String(data.get("died") ?? ""); const connectedTo = String(data.get("anchor")); const relation = String(data.get("relation")); if (!name) { setError("Enter a name."); return; } if (!living && born && died && Number(died) < Number(born)) { setError("Death year must be after birth year."); return; }
    ...
    if (await update(s => { let links = [...s.links]; const connect = (kind: "parent" | "partner", from: string, to: string) => links.push({ id: crypto.randomUUID(), kind, from, to });
